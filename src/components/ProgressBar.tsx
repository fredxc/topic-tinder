    ...
    <div className="w-full px-5 pt-1">
      <div className="flex items-center justify-between pb-1">
        <span className="text-xs text-rose-300/70">
          {swipeCount} foto{swipeCount !== 1 ? 's' : ''} avaliada{swipeCount !== 1 ? 's' : ''}
        </span>
        {isReady ? (
          <span className="text-xs text-emerald-400">Perfil desbloqueado ✓</span>
        ) : (
      ...
      <div className="h-1 w-full overflow-hidden rounded-full bg-white/10">
        <motion.div
          className={`h-full rounded-full ${isReady ? 'bg-emerald-400' : 'bg-rose-400'}`}
          initial={{ width: 0 }}
          animate={{ width: `${pct}%` }}
