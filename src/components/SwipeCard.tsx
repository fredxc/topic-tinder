...
const SwipeCard = forwardRef<HTMLDivElement, Props>(function SwipeCard({ photo, onSwipe, stackIndex }, forwardedRef) {
  ...
  return (
    ...
    >
      <div className="relative h-full w-full overflow-hidden rounded-3xl card-shadow">
        ...
        >
          <span className="font-playfair text-2xl font-bold italic tracking-wider">SIM ♥</span>
        </motion.div>
        ...
        >
          <span className="font-playfair text-2xl font-bold italic tracking-wider">NÃO ✕</span>
        </motion.div>

        {/* Bottom info */}
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <p className="mb-2 font-playfair text-lg font-medium italic text-white text-shadow">
            {photo.altDescription}
          </p>
          <p className="text-xs text-white/50">
            Foto por{' '}
            <a
              href={`${photo.photographerUrl}?utm_source=wedding_tinder&utm_medium=referral`}
              target="_blank"
