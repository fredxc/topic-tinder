...
import { dimensionBreakdown, DIMENSION_LABELS, DIMENSION_ICONS } from '../services/classifier'
...
export default function ResultScreen() {
  ...
  const { archetype, isHybrid, hybridName, normalizedScores, likedPhotos } = profile
  const breakdown = dimensionBreakdown(profile.scores)
  const displayName = isHybrid && hybridName ? hybridName : archetype.name
  const boardPhotos = visionPhotos.length > 0 ? visionPhotos : likedPhotos.slice(0, 6)

  return (
    <div className="relative h-dvh overflow-y-auto overflow-x-hidden bg-gradient-to-b from-rose-950 via-rose-900 to-rose-800 scrollbar-hide">
      <FloatingHearts />
          ...
          >
            {Array.from({ length: 30 }, (_, i) => (
              <motion.div
                ...
                transition={{ duration: 2 + Math.random() * 1.5, delay: Math.random() * 0.8 }}
              >
      ...
      <div className="relative z-10 mx-auto max-w-lg px-5 pb-16 pt-safe">
        ...
        <Section>
          <div className="pt-12 text-center">
            ...
            <p className="mb-1 text-xs font-medium uppercase tracking-widest text-rose-300">
              Seu perfil de casamento
            </p>

            <h1 className="font-playfair text-3xl font-bold italic leading-tight text-white text-shadow-lg">
              {displayName}
            </h1>
            ...
            <p className="mt-3 font-playfair text-base italic text-rose-200 text-shadow">
              "{archetype.tagline}"
            </p>
          </div>
        </Section>
        ...
        <Section>
          <div className="rounded-2xl bg-white/10 p-5 backdrop-blur-sm">
            <h2 className="mb-3 font-playfair text-lg font-semibold italic text-white">
              Quem é você como noiva
            </h2>
            <p className="text-sm leading-relaxed text-rose-100">{archetype.description}</p>
          </div>
        </Section>
              ...
              .map((dim) => (
                <motion.div key={dim} variants={fadeUp} className="mb-3">
                  <div className="mb-1 flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-sm text-rose-100">
                      <span>{DIMENSION_ICONS[dim]}</span>
                      <span>{DIMENSION_LABELS[dim]}</span>
                    </span>
                    <span className="text-sm font-semibold text-white">{breakdown[dim]}%</span>
                  </div>
          ...
          <div className="flex gap-3">
            {archetype.palette.map((swatch, i) => (
              ...
              >
                ...
                />
                <span className="text-center text-xs leading-tight text-rose-200">{swatch.name}</span>
              </motion.div>
            ))}
          </div>
        </Section>
          ...
          >
            {archetype.characteristics.map((c) => (
              ...
              >
                ...
                <span className="text-sm text-rose-100">{c}</span>
              </motion.div>
            ))}
          </motion.div>
        </Section>
        ...
        <Section>
          <div className="grid grid-cols-1 gap-4">
            <div className="rounded-2xl bg-white/10 p-4">
              <h3 className="mb-3 flex items-center gap-2 font-playfair font-semibold italic text-white">
                <span>🏛️</span> Locais ideais
              </h3>
              <ul className="space-y-1.5">
                {archetype.venueTypes.map((v) => (
                  <li key={v} className="flex items-center gap-2 text-sm text-rose-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-2xl bg-white/10 p-4">
                ...
                <ul className="space-y-1.5">
                  {archetype.flowerSuggestions.map((f) => (
                    <li key={f} className="text-xs text-rose-100">
                      · {f}
                    </li>
                  ))}
                </ul>
              ...
              <div className="rounded-2xl bg-white/10 p-4">
                <h3 className="mb-2 font-playfair font-semibold italic text-white">
                  💰 Orçamento
                </h3>
                <p className="text-sm font-semibold text-rose-200">{archetype.budgetRange}</p>
                <p className="mt-1 text-xs text-rose-300/70">estimativa média</p>
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section>
          <div className="text-center text-rose-200">
            <p className="font-playfair italic">
              Você amou{' '}
              <span className="font-bold text-white">{likedPhotos.length}</span>{' '}
              de{' '}
              <span className="font-bold text-white">{profile.totalSwipes}</span>{' '}
              fotos
            </p>
          </div>
        </Section>
        ...
        <Section>
          <div className="text-center">
            <motion.button
              ...
              className="rounded-full bg-white px-8 py-3.5 font-semibold text-rose-700 shadow-lg transition-shadow hover:shadow-xl"
            >
              Recomeçar do início
            </motion.button>
