...
export default function App() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (state.phase !== "swiping" || state.isMenuOpen || state.isHistoryOpen)
        return;
  ...
  return (
    <div className="flex h-dvh items-center justify-center overflow-hidden bg-stone-950">
      ...
      <div className="relative h-full w-full max-w-[430px] overflow-hidden">
              ...
              >
                {state.isFetching ? (
                  <>
                    ...
                    <p className="font-playfair italic text-stone-300">
                  ...
                  <>
                    ...
                    <p className="font-playfair text-lg italic text-stone-300">
                      Você viu todas as fotos!
                    </p>
                    <button
                      onClick={() => dispatch({ type: "GENERATE_RESULT" })}
                      className="mt-2 rounded-full bg-white px-6 py-3 font-semibold text-stone-800 shadow-lg"
                    >
                      Ver meu perfil
                    </button>
                  </>
        ...
        {/* Top overlay: title + menu + progress */}
        <div className="absolute top-0 left-0 right-0 z-20">
          <header className="flex items-center justify-between px-5 pb-1 pt-safe pt-4">
            <div className="w-10" /> {/* spacer */}
            <div className="text-center">
              <h1 className="font-playfair text-xl font-bold italic text-white drop-shadow-lg">
                Wedding Tinder
              </h1>
            </div>
            <MenuDots />
        ...
        {state.history.length === 0 && !noPhotosLeft && (
          <motion.div
            ...
            className="absolute bottom-32 left-0 right-0 z-20 flex items-center justify-center gap-6 text-sm text-white/50"
          >
            <span>← Não</span>
